...
The attributes and methods for an object of the class are accessed via dot notation. `bit_length()` is a method defined in `int` class, so, we can use this method of the `int` class on the object using `.`.
...
We can find the methods and attributes using `dir()` built-in python function.
